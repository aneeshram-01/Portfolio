  ...
  return (
    <div className="cookie-banner" role="dialog" aria-label="Cookie notice">
      <span className="cookie-text">
        <strong>Mandatory cookie notice.</strong> This site uses zero cookies,
        stores no data, and tracks nothing whatsoever. We were legally required
        to show this. Or maybe not. Either way — Hi.
      </span>
      <button className="cookie-btn" onClick={onDismiss}>
        Got it, obviously
      </button>
    </div>
  );
    ...
    <div className="sys-notif" role="dialog" aria-label="System notification">
      ...
      <div className="sys-notif-body">
        ...
        <div className="sys-notif-msg">
          This notification is entirely ceremonial. All systems nominal. No
          issues detected. You may dismiss it at any time. We recommend doing so
          immediately.
        </div>
      </div>
    </div>
...
// Shown when a mobile user tries to switch to desktop mode.
// Mode never actually switches — we intercept before it happens.
// Only one action: back to web. Backdrop click + Escape also dismiss.
function MobileDesktopWarningModal({ onClose }: { onClose: () => void }) {
  useEffect(() => {
  ...
  return (
    <div
      className="mobile-block-overlay"
      onClick={(e) => {
        if (e.target === e.currentTarget) onClose();
      }}
    >
      <div
        className="mobile-block-card"
        role="dialog"
        aria-modal
        aria-label="Desktop mode unavailable on mobile"
      >
        <div className="mobile-block-header">
          <div className="mobile-block-icon">A</div>
          <div className="mobile-block-appname">aneesh.dev</div>
          <div className="mobile-block-badge">desktop</div>
        </div>
        <div className="mobile-block-body">
          <div className="mobile-block-title">Open on a real screen</div>
          <div className="mobile-block-msg">
            The desktop experience — draggable windows, a real terminal, the
            full workstation — is built for a laptop or desktop. You&apos;re on
            mobile, so Web mode has everything you need right here.
          </div>
        </div>
        <div className="mobile-block-actions">
          <button className="mobile-block-btn" onClick={onClose}>
            Back to Web mode
          </button>
        </div>
      </div>
...
export default function App() {
  ...
  const [showMobileDesktopModal, setShowMobileDesktopModal] = useState(false);

  // Intercept desktop-mode requests on mobile — never actually switch.
  // Read window.innerWidth directly to avoid stale closure over isMobile state.
  const setMode = (m: "os" | "web") => {
    if (m === "os" && window.innerWidth <= 720) {
      setShowMobileDesktopModal(true);
      return;
    }
    _setMode(m);
    try {
      localStorage.setItem("aneesh:mode", m);
    } catch {}
  };
  ...
  useEffect(() => {
    const mobile = window.innerWidth <= 720;
    if (mobile && localStorage.getItem("aneesh:nudged") !== "1")
      setShowNudge(true);
  ...
  useEffect(() => {
    ...
    // Wallpaper via data attribute (not class) to avoid .wp-grid layout collision.
    // Desktop renders its own .wallpaper div; web mode uses body[data-wallpaper] CSS.
    const wp = t.showGrid ? "grid" : t.wallpaper;
    document.body.dataset.wallpaper = mode === "web" ? wp : "";
  ...
  useEffect(() => {
    ...
    function onKey(e: KeyboardEvent) {
      ...
      if (k === "e") {
        e.preventDefault();
        setMode(mode === "os" ? "web" : "os");
      } else if (k === "d") {
        e.preventDefault();
        setTweak("dark", !t.dark);
      }
      // ⌘+Alt+T (Mac) / Ctrl+Alt+T (Win) — open Terminal; no browser conflict
      else if (
        (e.metaKey || e.ctrlKey) &&
        e.altKey &&
        k === "t" &&
        ...
        e.preventDefault();
        window.__desktopApi?.openApp("terminal");
      } else if (k === "," && mode === "os") {
        e.preventDefault();
        window.__desktopApi?.openApp("settings");
      } else if (k === "." && mode === "os") {
        e.preventDefault();
        window.__desktopApi?.reorganizeIcons();
      }
    }
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  ...
  return (
    <>
      ...
      {showMobileDesktopModal && (
        <MobileDesktopWarningModal
          onClose={() => setShowMobileDesktopModal(false)}
        />
      )}
      ...
      {showNudge && (
        <MobileNudge
          onDismiss={() => {
            setShowNudge(false);
            localStorage.setItem("aneesh:nudged", "1");
          }}
          onSwitchToWeb={() => {
            setMode("web");
            setShowNudge(false);
      ...
      {mode === "web" && showCookie && (
        <CookieBanner
          onDismiss={() => {
            setShowCookie(false);
            localStorage.setItem("aneesh:cookie", "1");
          }}
        />
      )}
      ...
      {mode === "os" && showSysNotif && (
        <SystemNotif
          onDismiss={() => {
            setShowSysNotif(false);
            localStorage.setItem("aneesh:sysnotif", "1");
          }}
        />
      )}
