...
declare global {
  interface Window {
    __desktopApi?: DesktopApi;
  }
}

// ── App registry ──────────────────────────────────────────────────
const APPS = [
  { id: "about", name: "About", glyph: "A", size: { w: 720, h: 590 } },
...
function findFreeCell(
  ...
  const toPos = (c: number, r: number) => ({
    x: GRID_PAD_X + c * GRID_COL,
    y: GRID_PAD_Y + r * GRID_ROW,
  });
...
function defaultIconPositions(): Record<string, { x: number; y: number }> {
  ...
  const deskH =
    (typeof window !== "undefined" ? window.innerHeight : 768) - 36 - 100;
...
function AppWindow({
  ...
  function startDrag(e: React.MouseEvent | React.TouchEvent, isTouch: boolean) {
    ...
    function move(ev: MouseEvent | TouchEvent) {
      ...
      onMove(
        Math.max(0, ox + p.clientX - startX),
        Math.max(0, oy + p.clientY - startY),
      );
    }
    function up() {
...
function DesktopIcon({
  ...
  function startDrag(e: React.MouseEvent | React.TouchEvent, isTouch: boolean) {
    ...
    function move(ev: MouseEvent | TouchEvent) {
      ...
      const nx = Math.max(
        GRID_PAD_X,
        Math.min(window.innerWidth - 90, ox + dx),
      );
      const ny = Math.max(
        GRID_PAD_Y,
        Math.min(window.innerHeight - 36 - 130, oy + dy),
      );
  ...
  return (
    <div
      ...
      style={{
        ...
        left: dp.x,
        top: dp.y,
        zIndex: dragPos ? 50 : undefined,
      }}
      onDoubleClick={() => onOpen(app.id)}
      onMouseDown={(e) => {
        if (e.button === 0) startDrag(e, false);
      }}
      onTouchStart={(e) => startDrag(e, true)}
    ...
    >
      <div className="glyph" aria-hidden="true">
        <span style={{ fontSize: app.id === "trash" ? 22 : 18 }}>
          {app.glyph}
        </span>
      </div>
      ...
      {menu.items.map((it, i) =>
        it === "---" ? (
          ...
          >
            <span>{it.label}</span>
            {it.shortcut && (
              <span className="ctxmenu-shortcut">{it.shortcut}</span>
            )}
          </button>
        ),
      )}
...
// ── Dock ──────────────────────────────────────────────────────────
const DOCK_APPS = APPS.filter(
  (a) => !["trash", "settings", "certs"].includes(a.id),
);
    ...
    <div className="dock" role="toolbar" aria-label="Dock">
      {DOCK_APPS.map((a) => (
        <div
          key={a.id}
          className={
            "dock-item" + (runningIds.includes(a.id) ? " running" : "")
          }
      ...
      <div
        className={
          "dock-item" + (runningIds.includes("settings") ? " running" : "")
        }
...
export default function Desktop({ wallpaper, themeApi }: Props) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("aneesh:iconPos", JSON.stringify(iconPos));
    } catch {}
  }, [iconPos]);
  ...
  const reorganizeIcons = useCallback(
    () => setIconPos(defaultIconPositions()),
    [],
  );
  ...
  const openApp = useCallback((id: string) => {
    setWindows((ws) => {
      ...
      const cy = Math.max(
        60,
        (window.innerHeight - meta.size.h) / 2 - 60 + offset,
      );
      return [
        ...ws,
        {
  ...
  const api = useMemo<DesktopApi>(
    () => ({
      ...
      setMode: themeApi.setMode,
      getMode: themeApi.getMode,
      setWallpaper: themeApi.setWallpaper,
      getWallpaper: themeApi.getWallpaper,
      setVariant: themeApi.setVariant,
      getVariant: themeApi.getVariant,
      setAccent: themeApi.setAccent,
      getAccent: themeApi.getAccent,
    }),
    [
      openApp,
      closeApp,
  ...
  function onDesktopContextMenu(e: React.MouseEvent) {
    ...
    setMenu({
      ...
      items: [
        ...
        {
          label:
            themeApi.getTheme() === "dark"
              ? "Switch to Light"
              : "Switch to Dark",
          shortcut: "⌘D",
          onClick: () =>
            themeApi.setTheme(
              themeApi.getTheme() === "dark" ? "light" : "dark",
            ),
        },
        {
          label: "Switch to Web mode",
          shortcut: "⌘E",
          onClick: () => themeApi.setMode("web"),
        },
        "---",
        {
    ...
    setMenu({
      ...
      items: [
        ...
        "---",
        {
          ...
          onClick: () =>
            setIconPos((p) => ({ ...p, [id]: defaultIconPositions()[id] })),
        },
  ...
  return (
    <div
      className={"desktop" + (bumpKey ? " refreshing" : "")}
      key={bumpKey}
    ...
    >
      ...
      {windows.map((w) => (
        <AppWindow
          ...
          onMinimize={() => minApp(w.id)}
          onResize={() => zoomApp(w.id)}
